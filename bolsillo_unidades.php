...
function acum_totales($tipo, $titulo_fila, $arr_nivel1, $arr_nivel2, $arr_nivel3, $arr_nivel4)
{
	...
	} else if ((strcmp($tipo, 'UNIDADES')) == 0) {
		$acum_niveles = [
			"titulo_fila" => $titulo_fila,
			"tot_mat" => $arr_nivel1['tot_mat'] + $arr_nivel2['tot_mat'] + $arr_nivel3['tot_mat'] + $arr_nivel4['tot_mat'],
			"tot_mat_pub" => $arr_nivel1['tot_mat_pub'] + $arr_nivel2['tot_mat_pub'] + $arr_nivel3['tot_mat_pub'] + $arr_nivel4['tot_mat_pub'],
	...
	} else if ((strcmp($tipo, 'USAER')) == 0) {
		$acum_niveles = [
			"titulo_fila" => $titulo_fila,
			"tot_mat" => $arr_nivel1['tot_mat'],
			"tot_mat_pub" => $arr_nivel1['tot_mat_pub'],
			"tot_mat_priv" => $arr_nivel1['tot_mat_priv'],
			"mat_h" => $arr_nivel1['mat_h'],
			"mat_h_pub" => $arr_nivel1['mat_h_pub'],
			"mat_h_priv" => $arr_nivel1['mat_h_priv'],
			"mat_m" => $arr_nivel1['mat_m'],
			"mat_m_pub" => $arr_nivel1['mat_m_pub'],
...
function arreglos_datos($tipo_tabla, $link, $ini_ciclo, $filtro_extra, $filtro_pub, $filtro_priv)
{
	...
	if ((strcmp($tipo_tabla, 'NIVEL_PANO')) == 0) {
		$estadisticas = [
			...
			"total_usbq_lic_sup" => $total_usbq_lic_sup,
			"total_posg_sup" => $total_posg_sup,
			"total_sedeq_sup" => $total_sedeq_sup,
			"total_sedeq_sistema" => $total_sedeq_sistema,
			"sedeq_doc_plant_msup" => $sedeq_doc_plant_msup,
			"total_sedeq_munic_sup" => $total_sedeq_munic_sup,
			"total_sedeq_munic_unidades_sup" => $total_sedeq_munic_unidades_sup
...
function encabezado_pan_dip10($ini_ciclo)
{
	...
	echo "<TABLE BORDER='1' width='100%' CLASS='tb_borde'>";
	echo "<TR>";
	echo "<TD ROWSPAN='2'>MUNICIPIO</TD>";
...
function fila_pan_dip10($municipio, $titulo_fila, $tipo_servicio, $nivel, $arr_datos)
{
...
function juntar_msup($titulo_fila, $arr_subnivel1, $arr_subnivel2)
{
...
function acum_unidades($link, $ini_ciclo, $filtro_pub, $filtro_priv, $filtro_extra, $titulo_fila, $arr_nivel1, $arr_nivel2)
{
	...
	//echo "###".$datos_filtro_s1[1]."<BR>";
	$datos_filtro_s2 = explode('\'', $datos_filtro_s1[1]);
	//echo "###".$datos_filtro_s2[1]."<BR>";
	$municipio = $datos_filtro_s2[1];

	if ((strcmp($municipio, '14')) == 0) {

		$arr_nivel2 = subnivel($link, "UNIDADES SUPERIOR", $ini_ciclo, "unidades_sup", " ", $filtro_pub, $filtro_priv);
		//echo "###QRO<BR>";
		$acum_niveles = [
			"titulo_fila" => $titulo_fila,
...
function panorama_dip10($nombre_munic, $link, $ini_ciclo, $sin_filtro_extra, $filtro_pub, $filtro_priv)
{

	$arr_datos_est = arreglos_datos("NIVEL_PANO", $link, $ini_ciclo, $sin_filtro_extra, $filtro_pub, $filtro_priv);
	/* fila_pan_dip10($nombre_munic,"INICIAL(Escolarizado)","TOTAL","total_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"INICIAL(Escolarizado)","GENERAL","total_gral_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"INICIAL(Escolarizado)","INDIGENA","total_ind_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"INICIAL(No Escolarizado)","TOTAL","total_sedeq_nesc_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"INICIAL(No Escolarizado)","COMUNITARIO","total_sedeq_comuni_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"INICIAL(No Escolarizado)","NO ESCOLARIZADO","total_sedeq_ne_ini",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"ESPECIAL (CAM)","TOTAL","total_usbq_esp_total",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"ESPECIAL (USAER)","TOTAL","total_sedeq_esp_usaer",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PREESCOLAR","TOTAL","total_pree",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PREESCOLAR","GENERAL","total_pree_gral",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PREESCOLAR","COMUNITARIO","total_comuni_pree",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PREESCOLAR","INDIGENA","total_ind_pree",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PRIMARIA","TOTAL","total_prim",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PRIMARIA","GENERAL","total_gral_prim",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PRIMARIA","COMUNITARIO","total_comuni_prim",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"PRIMARIA","INDIGENA","total_ind_prim",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"SECUNDARIA","TOTAL","total_sec",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"SECUNDARIA","GENERAL","total_subn_gral_sec",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"SECUNDARIA","COMUNITARIO","total_comuni_sec",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"SECUNDARIA","TECNICA","total_tec_sec",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"SECUNDARIA","TELESECUNDARIA","total_tele_sec",$arr_datos_est); 
	//fila_pan_dip10($nombre_munic,"BASICA","BASICA","total_sedeq_basica",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"MEDIA SUPERIOR","TOTAL","total_sedeq_msup",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"MEDIA SUPERIOR","BACHILLERATO GENERAL","total_bgral_msup",$arr_datos_est);
	fila_pan_dip10($nombre_munic,"MEDIA SUPERIOR","BACHILLERATO TECNOLOGICO","total_btecno_msup",$arr_datos_est);
	 */
	fila_pan_dip10($nombre_munic, "SUPERIOR", "Licenciatura (TSU y Lic)", "total_lic_sup", $arr_datos_est);
	fila_pan_dip10($nombre_munic, "SUPERIOR", "TSU", "total_usbq_tsu_sup", $arr_datos_est);
	fila_pan_dip10($nombre_munic, "SUPERIOR", "LIC", "total_usbq_lic_sup", $arr_datos_est);
	fila_pan_dip10($nombre_munic, "SUPERIOR", "Posgrado", "total_posg_sup", $arr_datos_est);
	if ((strcmp($sin_filtro_extra, ' ')) == 0) {
		fila_pan_dip10($nombre_munic, "SUPERIOR MUNI", "TOTAL", "total_sedeq_munic_sup", $arr_datos_est);
		fila_pan_dip10($nombre_munic, "SUPERIOR 911", "TOTAL", "total_sedeq_sup", $arr_datos_est);
		fila_pan_dip10($nombre_munic, "SISTEMA", "SISTEMA", "total_sedeq_sistema", $arr_datos_est);

		//echo '####ESTATAL';
	} else {
		fila_pan_dip10($nombre_munic, "SUPERIOR MUNI", "TOTAL", "total_sedeq_munic_sup", $arr_datos_est);
		fila_pan_dip10($nombre_munic, "SUPERIOR MUNI UNIDADES", "TOTAL", "total_sedeq_munic_unidades_sup", $arr_datos_est);
		fila_pan_dip10($nombre_munic, "SUPERIOR 911", "TOTAL", "total_sedeq_sup", $arr_datos_est);
	}
...
<BODY>
	...
	/* 		encabezado_pan_dip10("24");
			panorama_dip10("ESTADO",$link,"24",$sin_filtro_extra,$filtro_pub,$filtro_priv);
			echo "</TABLE>";
			echo "<BR>"; */
	...
	for ($num_munic = 1; $num_munic <= 18; $num_munic++) {
		...
		$nombre_munic = nombre_municipio($num_munic);
		//echo $nombre_munic;
		//fila_pan_dip10($nombre_munic,"TOTAL","TOTAL","total_sedeq_sistema",arreglos_datos("NIVEL_PANO",$link,"24",$filtro_mun,$filtro_pub,$filtro_priv));
		panorama_dip10($nombre_munic, $link, "24", $filtro_mun, $filtro_pub, $filtro_priv);
